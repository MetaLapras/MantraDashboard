    ...
    && isset($_POST['type']))
{
    ...
    $details =$db->insertAddress($contact1, $contact2, $address_line_1, $address_line_2, $city, $pincode, $state, $country, $worker_id, $type,$employee_id);
        ...
        if($details)//!= false)
        {
            ...
            $contact_details=new ContactDetails(
                ...
                {
                    $db->setPermanentAddressIdByWorkersId($details["id"],$details["worker_id"]);
                }
                elseif($details['type']=='current')
                {
                    $db->setCurrentAddressIdByWorkersId($details["id"],$details["worker_id"]);
                }
                elseif($details['type']=='both')
                {
                    $db->setPermanentAddressIdByWorkersId($details["id"],$details["worker_id"]);
                    $db->setCurrentAddressIdByWorkersId($details["id"],$details["worker_id"]);
                }
